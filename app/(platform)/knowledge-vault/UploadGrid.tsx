...
function UploadTile({
  ...
  const route = routing[title] ?? {
    system: "vault" as KnowledgeSystem,
  ...
  const ingest = async (file: File) => {
    ...
    try {
      ...
      if (TEXT_EXT.includes(ext)) {
        ...
        fd.append("file", file);
        const parsed = await fetch("/api/frameworks/parse-file", {
          method: "POST",
          body: fd,
        }).then((r) => r.json());
        if (!parsed.success)
          throw new Error(parsed.error || "Could not read file");
    ...
    <>
      <button
        ...
        /* Two shapes, one card.
         ...
         Centred tile from `sm` up, where 200px of column is enough for a
         title to sit on one line under its icon. Below that a phone gives
         each tile about 150px, and "Upload Creative Framework" came out as
         three centred lines above a two-line file hint — eight tiles of
         ragged text. So on a phone it lies down: icon on the left, title and
         hint stacked beside it, one per row. It reads faster AND the whole
         grid ends up shorter than the two-column version it replaces. */
        className={`glass glass-hover group flex h-full flex-row items-center gap-3 rounded-xl border-dashed p-4 text-left transition-all sm:flex-col sm:items-center sm:justify-center sm:gap-2 sm:p-5 sm:text-center ${
          drag ? "border-glow shadow-glow" : "border-border"
        }`}
      >
        <span className="panel-icon acc-blue grid h-11 w-11 shrink-0 place-items-center rounded-lg transition-transform group-hover:scale-110">
          {status.kind === "working" ? (
        ...
        <span className="flex min-w-0 flex-col items-start gap-0.5 sm:contents">
          ...
          {status.kind === "working" && (
            <span className="text-[12.5px] font-medium text-glow">
              Ingesting…
            </span>
          )}
          {status.kind === "done" && (
            <span className="max-w-full truncate text-[12.5px] font-medium text-success">
              {status.stored
                ? `Ingested · ${status.chunks} chunk${status.chunks === 1 ? "" : "s"}`
                : "Queued (demo mode)"}
            </span>
          )}
          {status.kind === "error" && (
            <span className="max-w-full truncate text-[12.5px] font-medium text-danger">
              {status.message}
            </span>
          )}
        </span>
    ...
    >
      ...
      >
        ...
        <p className="mt-1.5 text-[12.5px] text-white/55">
          It is embedded and filed under {destination}, then retrievable by
          every agent.
        </p>
